...
var has = {
  ...
  return function(actual){
    if(actual == "")
      {
        return{
          message: "There is no element with id "+expected,
    ...
    class: function(expected){
      return function(actual){
        if(actual === null)
          {
            return {
              message: "The Element you want to test is NULL",
              success: false
            };
          }
        if(!expected)
          {
            return {
              message: "The element "+actual.tagName+" has got no Attribute class with value "+expected,
              success: (actual.hasAttribute()) ? true : false
            };
          }
          return{
            message: "The class of "+actual.tagName+" is not like "+expected,
            success: (actual.getAttribute("class") == expected) ? true : false
          };
