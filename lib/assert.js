  ...
  return function () {
    ...
    return function (actual) {
      ...
      test.message = (test.message.match("to")) 
                        ...
                        : test.message.replace('has', 'has no');
...
var has = {
  listItem: function(expected){
    return function(actual){
      if(actual == null || actual == "" || !actual.nodeType){
        return{
            ...
            for(j in liChilds)
              {
                if(liChilds[j].nodeValue == expected){
                  ...
                  return {
                    message: util.format("%s(id: "+actual.getAttribute('id')+") should has listItem like %s",actual.tagName,expected),
                    success: true
                  };
                }
              }
  ...
  child:  function(expected)
  {
    return function(actual){
      ...
      for(i in childs)
      {
        ...
        {
          return {
            message: util.format("%s (id: "+actual.getAttribute('id')+") should have a Child like %s",actual.tagName,expected.tagName),
            success: true
          }
        }
      }
