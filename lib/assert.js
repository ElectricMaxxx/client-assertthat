  ...
  attribute: function(expected,attr){
    ...
    return function(actual){
      ...
      if(!expected)
        {
          return {
            message: "The element "+actual.tagName+" has got no attribute "+attr+" with value "+expected,
            success: (actual.hasAttribute(attr)) ? true : false
          };
        }
        return{
          message: "The "+attr+" of "+actual.tagName+" is not like "+expected,
          success: (actual.getAttribute(attr) == expected) ? true : false
        };             
    };
  },
